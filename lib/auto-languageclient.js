...
export default class AutoLanguageClient {
  ...
  setBusyMessage(title: string): void {
    if (title == null && this._busyMessage != null) {
      this._busyMessage.dispose();
      this._busyMessage = null;
      return;
    }

    if (this._busyMessage != null) {
      this._busyMessage.setTitle(title);
      return;
    }

    if (this._busySignalService != null) {
      this._busyMessage = this._busySignalService.reportBusy(title);
    }
  }
}
